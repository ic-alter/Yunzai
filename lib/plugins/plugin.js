  ...
  setContext(type, isGroup, time = 120, timeout = "操作超时已取消") {
    ...
    if (time)
      stateArr[key][type][SymbolTimeout] = setTimeout(() => {
        ...
        if (resolve) return resolve(false)
        // Allow callers that need to continue an operation after a timeout to
        // provide their own handler, while retaining the string reply API.
        if (typeof timeout === "function") {
          Promise.resolve(timeout.call(this)).catch(err => globalThis.logger?.error?.(err))
          return
        }
